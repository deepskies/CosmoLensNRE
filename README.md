...
If you wish to build a pdf from a markdown file, as an easy way to share a compiled paper, you may use the `build-pdf` action. 
This action takes the `paper.md` file in `/paper/` and converts it into a latex style PDF. 
If you wish to add extra metadata, change the way the paper is formatted, etc, this can be done by changing this action `pandoc` command. 
