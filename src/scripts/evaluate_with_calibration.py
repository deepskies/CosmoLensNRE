...
class PosteriorCoverage:
    ...
    def log_likelihood(self, theta, data, aparam, w_low, w_high, om_low, om_high):
        ...
        # ll = 0.1*np.sum(logr_array) + 4.0
        ll = self.alpha*np.sum(logr_array) + self.beta
        return ll + lp
...
def plot_true_vs_mean_std_func(true_val, mean_val, std_val, xlabel, y1label, y2label, rchisq, alpha, beta, output_path):
    """
    ...
    plt.tight_layout()
    plt.savefig(output_path+f"parity_mcmc_calibrate_alpha{alpha}_beta{beta}.pdf", dpi=300)
    return 
...
def main():
    ...
    parser.add_argument('--alpha', type=float, default=0.1, help='calibration coefficient for log r')
    parser.add_argument('--beta', type=float, default=4.0, help='calibration coefficient for log r')
    ...
    if not os.path.exists(posterior_path+f'{args.data_dir}_mcmc_calibration_alpha{str(args.alpha)}_beta{str(args.beta)}.npz'):
        ...
        for i in range(0,n_pairs):
            try:
                ...
                posterior_coverage = PosteriorCoverage(args.model_path, args.model_name, 'fake_path', w_column_name, om_column_name, zl_column_name, zs_column_name, v_column_name, w_min, w_max, om_min, om_max, alpha, beta)
        ...
        # Save the samples
        np.savez(posterior_path+f'{args.data_dir}_mcmc_calibration_alpha{str(alpha)}_beta{str(beta)}.npz', flat_samples_list = flat_samples_list,
        ...
        posterior_data = np.load(posterior_path+f'{args.data_dir}_mcmc_calibration_alpha{str(args.alpha)}_beta{str(args.beta)}.npz')
    ...
    plt.tight_layout()
    plt.savefig(f"{args.output_path}coverage_plot_mcmc_calibrate_alpha{str(args.alpha)}_beta{str(args.beta)}.pdf", dpi=300)
    ...
    filename=f"{args.output_path}joint_posterior_mcmc_calibrate_alpha{str(args.alpha)}_beta{str(args.beta)}.pdf",)
